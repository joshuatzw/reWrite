...
pub fn build_system_prompt(config: &SkillsConfig, skill_id: Option<&str>) -> String {
    ...
    let combined = match (global.is_empty(), skill_instr.is_empty()) {
        (true, true) => {
            return "Rewrite the following text to improve clarity and flow. Return only the rewritten text.".to_string()
        }
